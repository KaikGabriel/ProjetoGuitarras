...
<body>
    ...
    <section class="bgCadastro">
        <div class="container my-5">
            <h2>Lista de Baterias Cadastrados</h2>
            <a class="btn btn-primary" href="{{'cadastrar-bateria'}}" role="button">Cadastrar</a>
            <br>
            <table class="table text-white">
                ...
                <tbody>
                    @foreach($registrosBateria as $registrosBaterias)
                    <tr>
                        <th>{{$registrosBaterias->id}}</th>
                        <td>{{$registrosBaterias->modelo}}</td>
                        <td>{{$registrosBaterias->marca}}</td>
                        <td>{{$registrosBaterias->valor}}</td>
                        <td>

                            <a class="btn btn-primary btn-sm" href="{{route('alterar-bateria',$registrosBaterias->id)}}">Editar</a>
                            <form method="Post" Action="{{route('apagar-bateria',$registrosBaterias->id)}}">
                                @method('delete')
                                @csrf
